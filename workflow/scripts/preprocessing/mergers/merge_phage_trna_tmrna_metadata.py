...
CONTRACT = load_contract(Path(__file__).resolve().parents[3] / "schemas" / "trna_tmrna_metadata_merged.yaml")
# List of DataFrames
dfs = []
...
for infile in inputs:
    ...
    df, _ = normalize_df_schema(df, CONTRACT, dataset_name="trna_tmrna_metadata", logger=logging.getLogger(__name__))
...
if dfs:
    ...
    final_schema_df, _ = normalize_df_schema(
        final_schema_df,
        CONTRACT,
        dataset_name="trna_tmrna_metadata_merged",
