...
COLUMNS_LIST = ["Protein_id", "Aligned_Protein_in_CARD", "Phage_ID", "Source_DB"]

NUMERICAL_COLUMNS = []

STRING_COLUMNS = ["Protein_id", "Aligned_Protein_in_CARD", "Phage_ID", "Source_DB"]
CONTRACT = load_contract(Path(__file__).resolve().parents[3] / "schemas" / "antimicrobial_resistance_gene_metadata_merged.yaml")
...
for infile in inputs:
    ...
    df, _ = normalize_df_schema(df, CONTRACT, dataset_name="antimicrobial_resistance_gene_metadata", logger=logging.getLogger(__name__))
...
if dfs:
    ...
    final_schema_df, _ = normalize_df_schema(
        final_schema_df,
        CONTRACT,
        dataset_name="antimicrobial_resistance_gene_metadata_merged",
