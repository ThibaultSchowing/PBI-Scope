...
CONTRACT = load_contract(Path(__file__).resolve().parents[3] / "schemas" / "transcription_terminator_metadata_merged.yaml")
...
for infile in inputs:
    ...
    df, _ = normalize_df_schema(df, CONTRACT, dataset_name="transcription_terminator_metadata", logger=logging.getLogger(__name__))
...
if dfs:
    ...
    final_schema_df, _ = normalize_df_schema(
        final_schema_df,
        CONTRACT,
        dataset_name="transcription_terminator_metadata_merged",
